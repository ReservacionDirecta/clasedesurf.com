  ...
  return (
    <article className="group flex flex-col rounded-3xl bg-white shadow-xl ring-1 ring-gray-100 transition-all duration-300 hover:-translate-y-2 hover:shadow-2xl">
      <div className="relative h-48 w-full overflow-hidden rounded-t-3xl">
        <Link href={`/schools/${id}`} className="block h-full w-full">
          <ImageWithFallback
            src={school.coverImage || '/hero2.png'}
            alt={name}
            fill
        ...
        <div className="absolute inset-0 bg-linear-to-t from-black/60 to-transparent opacity-60 pointer-events-none" />
      ...
      <div className="relative flex flex-1 flex-col px-6 pb-6 pt-12">
        {/* Logo Overlay */}
        <Link href={`/schools/${id}`} className="absolute -top-10 left-6 h-20 w-20 overflow-hidden rounded-2xl border-4 border-white bg-white shadow-lg block transition-transform hover:scale-105">
          {logo ? (
            <ImageWithFallback
              src={logo}
              alt={`${name} logo`}
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="mt-auto flex flex-col gap-2 pt-2">
            <Link 
              href={`/schools/${id}`}
              className="flex w-full items-center justify-center gap-2 rounded-xl bg-linear-to-r from-[#2EC4B6] to-[#5DE0D0] px-4 py-3 text-sm font-bold text-white shadow-lg transition-all duration-300 group-hover:shadow-xl"
            >
              Ver perfil completo
              <svg className="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
