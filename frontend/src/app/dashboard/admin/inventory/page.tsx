...
import { Package, Plus, Edit, Trash2, Search, Box, Building } from 'lucide-react';
...
export default function InventoryPage() {
  ...
  const fetchSchools = useCallback(async () => {
    try {
      ...
      const res = await fetch(buildBackendUrl('/schools'), { headers });
      if (res.ok) {
  ...
  const fetchProducts = useCallback(async () => {
    try {
      ...
      if (selectedSchoolFilter !== 'all') {
        url = `/products/school/${selectedSchoolFilter}`;
      }

      const response = await fetch(buildBackendUrl(url), { headers });
  ...
  }, [session, selectedSchoolFilter]);

  useEffect(() => {
    if (status === 'loading') return;
    if (!session || session.user.role !== 'ADMIN') {
      // Redirect if not admin (though middleware should handle this)
      // For now just allow render if role check passes
    }
    fetchSchools();
    fetchProducts();
  }, [session, status, fetchSchools, fetchProducts]);
  ...
  const handleCreateProduct = async (formData: FormData) => {
    ...
    try {
      ...
      formData.append('schoolId', createSchoolId);
    ...
    <div className="min-h-screen bg-gray-50 pb-20 sm:pb-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="sm:flex sm:items-center sm:justify-between mb-8">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Gestión de Inventario (Admin)</h1>
            <p className="mt-1 text-sm text-gray-500">Administra los productos de todas las escuelas.</p>
          </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                  ...
                  <div className="p-4">
                    <div className="mb-2">
                         <div className="flex items-center text-xs text-gray-500 mb-1">
                             ...
                             <span className="truncate">{product.school?.name || 'Sin Escuela'}</span>
                         </div>
                    </div>
              ...
              {filteredProducts.length === 0 && (
                <div className="col-span-full py-12 text-center bg-white rounded-xl border border-dashed border-gray-300">
                   ...
                   <p className="text-gray-500">Agrega productos para empezar a vender.</p>
                </div>
              )}
            </div>
        )}
        ...
        {showCreateModal && (
          <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
             <div className="bg-white rounded-2xl p-6 max-w-lg w-full max-h-[90vh] overflow-y-auto">
                ...
                {/* School Selector inside form context */}
                <div className="mb-4">
