...
export default function AdminNotificationDetailPage({ params }: { params: { id: string } }) {
  ...
  const loadNotification = async () => {
    try {
      ...
      // router.push('/dashboard/admin/notifications'); // Redirect if not found
    } finally {
      setLoading(false);
    }
  };
  ...
  if (!notification) {
    return (
      <div className="p-8 text-center">
        ...
        <Link href="/dashboard/admin/notifications" className="text-blue-600 mt-4 inline-block hover:underline">
          Volver a la lista
        </Link>
      </div>
    );
    ...
    <div className="p-4 max-w-4xl mx-auto">
      <div className="mb-6 flex items-center justify-between">
        <Link 
          ...
          className="inline-flex items-center px-4 py-2 bg-white border border-gray-300 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-50 hover:text-purple-600 transition-all shadow-sm group"
        >
