...
async function main() {
    try {
        ...
        if (!admin) {
            ...
            admin = await prisma.user.create({
                data: {
                    ...
                    name: 'Debug Admin',
                    password: 'hashes_are_hard', // Won't login with this, but needed for schema
                    role: 'ADMIN'
                }
            });
        }
        ...
        const payload = {
            ...
            // Using a valid school ID if possible
            schoolId: 1,
            // Trying beachId 1. If it fails due to FK, we'll see the error.
            beachId: 1,
            occurrences: [
